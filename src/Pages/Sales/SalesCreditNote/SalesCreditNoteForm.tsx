...
const SalesCreditNoteForm = () => {
  ...
  const permission = usePagePermission(PAGE_TITLE.SALES.SALES_CREDIT_NOTE.BASE);
  const { data: routeData } = location.state || {};

  const isEditing = Boolean(routeData?._id);
  const pageMode = isEditing ? "EDIT" : "ADD";

  const { data: singleData } = Queries.useGetSingleSalesCreditNote(routeData?._id);
  ...
  return (
    <>
      <CommonBreadcrumbs title={PAGE_TITLE.SALES.SALES_CREDIT_NOTE[pageMode]} maxItems={3} breadcrumbs={BREADCRUMBS.SALES_CREDIT_NOTE[pageMode]} />
