...
const AccountTab = () => {
  ...
  const [form, setForm] = useState({ branchId: "", businessAccountId: "", phoneNumberId: "", displayPhoneNumber: "", accessToken: "", graphVersion: "v23.0" });
  ...
  const handleSave = () => {
    ...
    upsertAccount.mutate(payload, { onSuccess: () => { setForm({ branchId: "", businessAccountId: "", phoneNumberId: "", displayPhoneNumber: "", accessToken: "", graphVersion: "v23.0" }); setEditingId(null); refetch(); } });
  };

  const handleEdit = (acc: MetaWhatsAppAccount) => {
    const bId = typeof acc.branchId === "object" ? (acc.branchId as any)._id : acc.branchId || "";
    setForm({ branchId: bId, businessAccountId: acc.businessAccountId, phoneNumberId: acc.phoneNumberId, displayPhoneNumber: acc.displayPhoneNumber || "", accessToken: acc.accessToken, graphVersion: acc.graphVersion || "v23.0" });
          ...
          <Grid container spacing={2}>
            <Grid size={{ xs: 12, md: 6 }}>
              <FormControl fullWidth>
                ...
                <Select value={form.branchId} onChange={(e) => setForm({ ...form, branchId: e.target.value })} label="Branch (optional)">
                  <MenuItem value="">None (All Branches)</MenuItem>
            ...
            <Grid size={{ xs: 12 }}>
              <Button variant="contained" startIcon={<WhatsApp />} onClick={handleSave} disabled={upsertAccount.isPending}>
      ...
      {accounts.length === 0 ? (
        ...
        <TableContainer component={Paper}>
          <Table>
            ...
            <TableBody>
              {accounts.map((row) => {
                const branchName = branches.find((b: any) => b._id === (typeof row.branchId === "object" ? (row.branchId as any)._id : row.branchId));
                return (
                  <TableRow key={row._id}>
                    <TableCell>{branchName ? branchName.displayName || branchName.name : "-"}</TableCell>
...
const TemplatesTab = () => {
  ...
  const initialState = { accountId: "", name: "", language: "en_US", category: "UTILITY", useFor: "CUSTOM", bodyText: "", sendAttachment: false, attachmentType: "pdf" };
  ...
  const handleCreate = () => {
    ...
    createTemplate.mutate({ accountId: form.accountId, name: form.name, language: form.language, category: form.category as any, useFor: form.useFor as any, components, sendAttachment: form.sendAttachment, attachmentType: form.attachmentType }, { onSuccess: () => { setShowForm(false); setForm({ ...initialState }); refetch(); } });
  };
...
const WhatsAppSetting = () => {
  ...
  return (
    <>
      <CommonBreadcrumbs title={PAGE_TITLE.WHATSAPP.BASE} maxItems={1} breadcrumbs={BREADCRUMBS.WHATSAPP?.BASE || [{ label: PAGE_TITLE.WHATSAPP.BASE }]} />
