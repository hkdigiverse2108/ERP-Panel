...
const ContactForm = () => {
  ...
  const handleSubmit = async (values: ContactFormValues, { resetForm }: FormikHelpers<ContactFormValues>) => {
    ...
    const payload: AddContactPayload = {
      ...rest,
      ...(rest.customerType === CONTACT_TYPE[0].value || rest.customerType === CONTACT_TYPE[1].value ? { supplierType: rest.supplierType } : {}),
              ...
              <Grid container spacing={2}>
                ...
                <CommonCard topContent={topContent} title="General Details" grid={{ xs: 12 }}>
                  <Grid container spacing={2} sx={{ p: 2 }}>
                    ...
                    <CommonTextField name="companyName" label="Company Name" grid={{ xs: 12, md: 4 }} />
                    ...
                    <CommonValidationSelect name="paymentTermsId" label="Payment Terms" options={GenerateOptions(paymentTermsData?.data)} isLoading={isPaymentTermsLoading || isPaymentTermsFetching} grid={{ xs: 12, md: 4 }} />
                    ...
                    {({ push, remove }) => (
                      <Box p={2}>
                        {values?.address?.map((_, index) => {
                          const isCustomer = values?.contactType === "customer";
                          return (
                            <Box key={index} mb={2} border="1px solid #ddd" p={2} borderRadius={1}>
                              <Grid container spacing={2}>
                                ...
                                <CommonTextField name={`address.${index}.gstIn`} label="GSTIN" required={isCustomer} disabled={values.address?.[index]?.gstType === "UnRegistered"} grid={{ xs: 12, md: 4 }} />
                                <CommonTextField name={`address.${index}.contactFirstName`} label="Contact First Name" required={isCustomer} grid={{ xs: 12, md: 4 }} />
