...
import { CommonValidationTextField as CommonTextField, CommonSwitch, CommonPhoneNumber, CommonRadio, CommonValidationSelect, CommonValidationRadio, CommonValidationDatePicker } from "../../Attribute";
...
const ContactForm = () => {
  ...
  const handleSubmit = async (values: ContactFormValues & { _submitAction?: "save" | "saveAndNew" }, { resetForm }: FormikHelpers<ContactFormValues>) => {
              ...
              <Grid container spacing={2}>
                ...
                <CommonCard topContent={topContent} title="General Details" grid={{ xs: 12 }}>
                  <Grid container spacing={2} sx={{ p: 2 }}>
                    ...
                    {contactType === "transporter" && <CommonTextField name="transporterId" label="Transport Id" required grid={{ xs: 12, md: 6 }} />}
                  </Grid>
                </CommonCard>
                  ...
                  <FieldArray name="addressDetails">
                    {({ push, remove }) => (
                      <Box p={2}>
                        {values?.addressDetails?.map((_, index) => {
                          ...
                          return (
                            <Box key={index} mb={2} border="1px solid #ddd" p={2} borderRadius={1}>
                              <Grid container spacing={2}>
                                <CommonTextField name={`addressDetails.${index}.gstType`} label="GST Type" required grid={{ xs: 12, md: 6 }} />
                                <CommonTextField name={`addressDetails.${index}.gstIn`} label="GSTIN" required grid={{ xs: 12, md: 6 }} />

                                <CommonTextField name={`addressDetails.${index}.contactFirstName`} label="Contact First Name" required grid={{ xs: 12, md: 6 }} />
                                <CommonTextField name={`addressDetails.${index}.contactLastName`} label="Contact Last Name" required grid={{ xs: 12, md: 6 }} />

                                <CommonTextField name={`addressDetails.${index}.contactCompanyName`} label="Company Name" required grid={{ xs: 12, md: 6 }} />

                                <CommonPhoneNumber label="Phone No." countryCodeName={`addressDetails.${index}.contactNo.countryCode`} numberName={`addressDetails.${index}.contactNo.phoneNo`} grid={{ xs: 12, md: 6 }} required />

                                <CommonTextField name={`addressDetails.${index}.contactEmail`} label="Email" required grid={{ xs: 12, md: 6 }} />

                                <CommonTextField name={`addressDetails.${index}.addressLine1`} label="Address Line 1" required grid={{ xs: 12, md: 6 }} />
                                <CommonTextField name={`addressDetails.${index}.addressLine2`} label="Address Line 2" required grid={{ xs: 12, md: 6 }} />
                                ...
                                <CommonTextField name={`addressDetails.${index}.pinCode`} label="Pin Code" required grid={{ xs: 12, md: 4 }} />

                                {(values?.addressDetails?.length || 0) > 1 && (
                                  <Grid size={12}>
                                    <button type="button" onClick={() => remove(index)} style={{ color: "red" }}>
                                      Remove Address
                                    </button>
                                  </Grid>
                                )}
                        ...
                        <button type="button" onClick={() => push({ gstType: "", gstIn: "", contactFirstName: "", contactLastName: "", contactEmail: "", addressLine1: "", addressLine2: "", country: "", state: "", city: "", pinCode: "", contactCompanyName: "", tanNo: "", contactNo: { countryCode: "", phoneNo: "" } })}>
                          + Add New Address
                        </button>
                      </Box>
                    )}
