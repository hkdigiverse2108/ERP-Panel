...
const FinancialYear = () => {
  ...
  const [selectedYear, setSelectedYear] = useState(company?.financialYear);

  const financialYearData = financialYear?.find((item) => item.value === selectedYear);
  return (
        ...
        <div className="p-3">
          <ul className="flex flex-col gap-3">
            {financialYear?.map((item, index) => (
              <li key={index} className="flex items-center gap-3 border-b pb-3 border-gray-100 dark:border-gray-800" onClick={() => setSelectedYear(item.value)}>
