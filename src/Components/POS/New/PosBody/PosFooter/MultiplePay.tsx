                  ...
                  {({ values }) => (
                    <Form>
                      <FieldArray name="multiplePayments">
                        {({ push, remove }) => {
                          const totalAmount = values.multiplePayments.reduce((total, pay) => total + (pay.amount ? Number(pay.amount) : 0), 0);
                          const remainingAmount = Number(PosProduct.totalAmount) - totalAmount;
                          return (
                            <div className="flex flex-col gap-3">
                              <div className="space-y-3 max-h-130! overflow-y-auto custom-scrollbar">
                                {values.multiplePayments.map((pay, index) => (
                                  <div key={index} className="bg-gray-50 dark:bg-gray-800 p-3 rounded">
                                    <Grid container spacing={2} alignItems="center">
                                      <CommonValidationTextField name={`multiplePayments.${index}.amount`} label="Received Amount" type="number" grid={{ xs: 12, sm: 4 }} />
                              ...
                              <div>
                                <CommonButton type="button" variant="text" title="➕ Add More Payment" disabled={totalAmount >= PosProduct.totalAmount} onClick={() => push({ amount: remainingAmount, method: "cash" })} />
                                ...
                                {totalAmount >= PosProduct.totalAmount ? (
                                  ...
                                  <p className="text-red-500 my-2 text-base">Note: If you don't pay in full, the remaining amount will be considered as Pay Later.</p>
                                )}
