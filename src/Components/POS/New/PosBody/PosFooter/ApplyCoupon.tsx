...
const ApplyCoupon = () => {
  ...
  // console.log("applyingId", applyingId);
  ...
  const handleApplyCoupon = (coupon: CouponBase) => {
    ...
    if (PosProduct.couponId === coupon.name) {
      ...
      dispatch(setTotalAmount(Number(PosProduct?.totalAmount) + Number(PosProduct.couponDiscount || 0)));
      setIsTotalAmount(Number(PosProduct?.totalAmount) + Number(PosProduct.couponDiscount || 0));
    ...
    verifyCoupon(
      { couponId: coupon._id, totalAmount: PosProduct?.totalAmount, customerId: PosProduct?.customerId },
      {
        onSuccess: (response) => {
          ...
          setIsTotalAmount(Number(response?.data?.finalAmount));
          setApplyingId("");
  ...
  console.log(PosProduct?.totalAmount, isTotalAmount);
  console.log(PosProduct?.totalAmount !== isTotalAmount);

  if (PosProduct?.totalAmount !== undefined && PosProduct?.totalAmount !== isTotalAmount) {
    setIsTotalAmount(PosProduct?.totalAmount);
    dispatch(setCoupon({ couponId: "", couponDiscount: 0 }));
    setApplyingId("");
    setApplyingId(PosProduct.couponId);
  }

  useEffect(() => {
    if (!PosProduct.posOrderId) return;

    const finalDiscount = Number(PosProduct.totalDiscount || 0) + Number(PosProduct.couponDiscount || 0);
