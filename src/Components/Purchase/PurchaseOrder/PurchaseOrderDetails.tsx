...
const PurchaseOrderDetails = () => {
    ...
    const { data: customerData, isLoading: supplierDataLoading, isFetching: supplierDataFetching } = Queries.useGetContactDropdown({ typeFilter: "supplier" });
    ...
    return (
        <Grid container spacing={2} sx={{ p: 2 }}>
            <Grid size={{ xs: 12, md: 3 }} sx={{ order: { xs: 1, md: 1 } }}>
                <CommonValidationSelect name="supplierId" label="Select Supplier" required isLoading={supplierDataLoading || supplierDataFetching} options={GenerateOptions(suppliers)} grid={{ xs: 12 }}  />
            </Grid>
