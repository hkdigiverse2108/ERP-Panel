...
const EstimateReceiptType2 = forwardRef<HTMLDivElement, { bill: any }>(({ bill }, ref) => {
    ...
    return (
        <div ref={ref} className="mx-auto w-[210mm] min-h-[297mm] bg-white text-black font-sans text-[12px] flex flex-col relative border border-black" style={{ fontFamily: "Arial, sans-serif" }}>
            ...
            <div className="flex border-t border-b border-black">
                <div className="w-[55%] border-r border-black flex flex-col">
                    <div className="font-bold text-[14px] px-2 py-1">To, {bill.customerId?.firstName ? `${bill.customerId.firstName} ${bill.customerId.lastName || ""}` : "Customer name"}</div>
                    <div className="text-[11px] text-gray-700 bg-[#eef5fa] px-2 py-2 flex-1 relative bottom-0">
                      {bill.customerId?.address?.[0]?.addressLine1 ? bill.customerId.address[0].addressLine1 : "Address"}<br />
                      {bill.customerId?.address?.[0]?.city?.name || "Ahmedabad"} - {bill.customerId?.address?.[0]?.pinCode || "123456"} {company?.address?.state?.name || "Gujarat India"}<br />
                      Mob No :{bill.customerId?.phoneNo?.phoneNo || "82123345678"}<br />
                      <span className="mt-1 inline-block">Company GSTIN :{(bill.customerId as any)?.gstin || "24CUSTM1206D1ZM"}</span>
                    </div>
                </div>
                <div className="w-[45%] flex flex-col font-bold text-[13px]">
                    <div className="text-center text-[15px] border-b border-black py-1 uppercase tracking-widest bg-gray-50 flex-none">
                        ESTIMATE
                    </div>
                ...
                <table className="w-full text-[11px] text-center table-fixed flex-1 h-full">
                    <thead className="h-8">
                        <tr className="border-b border-black font-bold h-full">
                            ...
                            <th className="border-r border-black py-1 px-2 w-[28%] text-left align-middle">Particular</th>
                            <th className="border-r border-black py-1 px-1 w-[13%] text-center align-middle">Brand Name</th>
                            ...
                            <th className="border-r border-black py-1 px-1 w-[7%] text-center align-middle">Discount</th>
                            <th className="border-r border-black py-1 px-1 w-[7%] text-center align-middle leading-tight">Discount 2</th>
                            <th className="border-r border-black py-1 px-1 w-[6%] text-center align-middle">Rate</th>
                            <th className="border-r border-black py-1 px-1 w-[8%] text-center align-middle leading-tight">Tax<br/>Amount</th>
                            <th className="py-1 px-1 w-[10%] text-center align-middle">Amount</th>
                        </tr>
                    </thead>
                        ...
                        {bill.items?.map((item: any, index: number) => {
                            ...
                            return (
                                <tr key={index} className="h-8">
                                    ...
                                    <td className="border-r border-black py-2 px-1">{Number(item.qty || 0).toFixed(2)}</td>
                                    <td className="border-r border-black py-2 px-1">{item.productId?.uomId?.name || item.unit || "UOM"}</td>
                                    <td className="border-r border-black py-2 px-1">{Number(disc1).toFixed(1)} %</td>
                                    <td className="border-r border-black py-2 px-1">{Number(disc2).toFixed(1)} %</td>
                                    <td className="border-r border-black py-2 px-1">{Number(rate).toFixed(0)}</td>
                    ...
                    <tfoot className="h-6">
                        <tr className="border-t border-black font-bold h-full">
                            <td colSpan={3} className="border-r border-black py-1 px-2 text-right"></td>
                            <td className="border-r border-black py-1 text-center align-middle text-[10px]">{Number(totalQty).toFixed(3)}</td>
                            <td colSpan={3} className="border-r border-black py-1 text-center align-middle"></td>
                            <td className="border-r border-black py-1 text-center align-middle text-[10px]">{Number(totalRate).toFixed(3)}</td>
                            <td className="border-r border-black py-1 text-center align-middle text-[10px]">{Number(totalTax).toFixed(3)}</td>
                            <td className="py-1 text-center align-middle text-[10px]">{Number(tableAmountAmt).toFixed(3)}</td>
                        </tr>
                    </tfoot>
                </table>
            ...
            <div className="flex border-t border-black font-bold">
                <div className="flex-1 py-1.5 px-3 border-r border-black text-center text-[12px]">
                    Rupees {inWords(Math.round(totalAmount))} Only
                </div>
                <div className="w-[12%] py-1.5 px-2 text-center text-[14px] border-l-0">
                    {Number(totalAmount).toFixed(3)}
                </div>
            </div>

            <div className="min-h-[100px] border-t border-black relative p-3 font-bold text-right text-[12px]">
                <div className="mt-1">For, {company?.name || "VasyERP Solution Private LTD"}</div>
                
                <div className="absolute bottom-6 left-3 text-left text-[10px]">
                    <div className="border-b border-black w-32 pb-0.5">Terms and Condition :</div>
                </div>
                ...
                <div className="absolute bottom-1 left-1/2 -translate-x-1/2 text-[10px] font-normal">
                    Page 1 of 1
                </div>
                <div className="absolute bottom-1 right-2 text-[10px] font-normal">
                    E.O.E.
                </div>
            </div>
        </div>
...
export default EstimateReceiptType2;
