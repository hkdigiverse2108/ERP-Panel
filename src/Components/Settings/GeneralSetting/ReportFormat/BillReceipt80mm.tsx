...
const BillReceipt80mm = forwardRef<HTMLDivElement, { bill: PosOrderBase }>(({ bill }, ref) => {
        ...
        <div ref={ref} id="last-bill-print" className="mx-auto w-[80mm] bg-white text-black p-4 font-mono text-[12px] leading-tight">
            ...
            <div className="flex justify-between mb-2 text-[11px]">
                <div className="flex flex-col">
                    ...
                    <div className="flex gap-1">
                        ...
                        <span>
                            : {bill?.customerId?.phoneNo?.countryCode ? `${bill?.customerId?.phoneNo?.countryCode}-` : ""}{bill?.customerId?.phoneNo?.phoneNo || "-"}
                        </span>
                    </div>
                </div>
                            ...
                            <React.Fragment key={index}>
                                ...
                                <tr>
                                    <td colSpan={5} className="pl-6 text-[9px] italic font-semibold pb-1">
                                        HSN: {item.productId?.hsnCode || "-"} GST@{getTaxPercent(item)}%
            ...
            <div className="mb-2 text-[11px]">
                {bill.additionalCharges?.length > 0 && (
                    <div className="flex justify-between border-b border-dashed border-black pb-1 mb-1">
                        ...
                        <span>: <span className="inline-block w-16 text-right font-bold">{Number(bill.additionalCharges?.reduce((acc, charge) => acc + (charge.totalAmount || 0), 0)?.toFixed(2) || 0)}</span></span>
                    </div>
                )}
                ...
                <div className="flex flex-col items-end pr-2 font-bold">
                    <div className="flex justify-between w-[50%]">
                        <span>TOTAL</span>
                        <span>: <span className="inline-block w-16 text-right">{Number(bill.totalAmount?.toFixed(2) || 0)}</span></span>
                    </div>
                    {bill.roundOff > 0 && (
                        <div className="flex justify-between w-[50%]">
                            <span>ROUND OFF</span>
                            <span>: <span className="inline-block w-16 text-right">{Number(bill.roundOff?.toFixed(2) || 0)}</span></span>
                        </div>
                    )}
                    {bill.multiplePayments?.map((payment, index) => (
                        <div className="flex justify-between w-[50%]" key={index}>
                            ...
                            <span>: <span className="inline-block w-16 text-right">{Number(payment.amount?.toFixed(2) || 0)}</span></span>
                        </div>
                    ))}
                </div>
            </div>
                ...
                <table className="w-full text-[10px] border border-black text-center">
                    <thead>
                        <tr className="border-b border-black font-bold">
                            <th className="border-r border-black p-1">TAXABLE<br />VALUE</th>
                    ...
                    <tbody>
                        {Object.keys(taxSummary).length > 0 ? (
                            ...
                            <tr>
                                <td colSpan={5} className="p-1 font-bold italic">No taxes applied</td>
                            </tr>
                        )}
                    </tbody>
                </table>
            ...
            <div className="text-[11px] mb-2">
                ...
                <div className="font-bold mt-1">
                    Address : {bill?.customerId?.address && bill.customerId.address.length > 0 ? `${bill.customerId.address[0]?.addressLine1 || ""}, ${bill.customerId.address[0]?.city?.name || ""}` : "-"}
                </div>
            </div>
            ...
            {/* Footer */}
            <div className="text-center font-bold text-[11px] mb-3">
...
export default BillReceipt80mm;
