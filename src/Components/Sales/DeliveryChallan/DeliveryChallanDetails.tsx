...
const DeliveryChallanDetails = ({ isEditing = false }: { isEditing?: boolean }) => {
  ...
  const { data: invoiceData, isLoading: isInvoiceLoading, isFetching: isInvoiceFetching } = Queries.useGetInvoiceDropdown({ customerId: values?.customerId, ...(isEditing && { includeId: values?.selectedInvoiceId?.join(",") }), statusFilter: "invoiced" }, !!values?.customerId);
