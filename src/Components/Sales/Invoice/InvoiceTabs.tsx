...
import { CommonShippingDetails, CommonTable, CommonTabPanel, CommonTermsAndCondition } from "../../Common";

const InvoiceTabs = ({ emptyRow, isEditing }: { emptyRow: InvoiceItem; isEditing: boolean }) => {
  ...
  const handleBulkAdd = async (file: File) => {
    try {
      ...
      const mappedItems = mapExcelToFormItems(rawData, BULK_IMPORT_TYPES.INVOICE, productsData?.data || [], emptyRow);

      const updatedItems = [...(values.items ?? []).filter((i: InvoiceItem) => i.productId), ...mappedItems];
  ...
  const deliveryChallanId = values?.selectedDeliveryChallanId?.[values?.selectedDeliveryChallanId?.length - 1] || "";
  ...
  const { data: singleDeliveryChallan, isLoading: isSingleDeliveryChallanLoading, isFetching: isSingleDeliveryChallanFetching } = Queries.useGetSingleDeliveryChallan(deliveryChallanId, Boolean(deliveryChallanId) && !isEditing);
  ...
  useEffect(() => {
    if ((!salesOrderId && !deliveryChallanId) || isEditing) return;
    ...
    if (deliveryChallanId && singleDeliveryChallan?.data) {
      const existingItems = (values?.items || []).filter((item) => item?.productId);
      const finalItems = [...existingItems, ...mapItems(singleDeliveryChallan?.data?.items)];
      setFieldValue("items", finalItems);
    }
  }, [singleSalesOrder, singleDeliveryChallan]);
  ...
  useEffect(() => {
    ...
    values?.items?.forEach((item, index) => {
      ...
      if (discountAmount > amount) {
        ...
        setFieldValue(`items.${index}.discountAmount`, 0);
      }
    });
  ...
  return (
    <>
      <Box sx={{ width: "100%" }}>
        <Box sx={{ borderBottom: 1, borderColor: "divider", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <Box sx={{ display: "flex", justifyContent: "flex-end", gap: 1, p: 1 }}>
            <CommonButton variant="contained" startIcon={<UploadFile />} title="Upload Products" size="small" disabled={!isCustomerSelected} onClick={() => dispatch(setBulkAddModal({ open: true, title: "Import Invoice", type: BULK_IMPORT_TYPES.INVOICE }))} />
          </Box>
        </Box>
                ...
                {({ push, remove }) => {
                  const columns: CommonTableColumn<any>[] = [
                      ...
                      render: (_, index) => (
                        <>
                          {index === ((values?.items || []).length || 0) - 1 && (
                            <CommonButton variant="outlined" onClick={() => push(emptyRow)}>
                    ...
                    },
                    {
                      ...
                      render: (_, index) => <CommonValidationTextField name={`items.${index}.qty`} type="number" />,
                    ...
                    },
                    {
                      ...
                      header: "Free Qty",
                      bodyClass: "min-w-28",
                      render: (_, index) => <CommonValidationTextField name={`items.${index}.freeQty`} type="number" />,
                    ...
                    },
                    {
                      ...
                      header: "Price",
                      bodyClass: "min-w-28",
                      render: (_, index) => <CommonValidationTextField name={`items.${index}.price`} type="number" />,
                    },
                    {
                      key: "discount1",
                      header: "Discount",
        ...
        <CommonTabPanel value={tabValue} index={1}>
          <CommonTermsAndCondition selectedTermIds={values.termsAndConditionIds || []} onChange={(ids: string[]) => setFieldValue("termsAndConditionIds", ids)} />
        </CommonTabPanel>
      ...
      <BulkAddModal type={BULK_IMPORT_TYPES.INVOICE} onUpload={handleBulkAdd} />
    </>
  );
};
