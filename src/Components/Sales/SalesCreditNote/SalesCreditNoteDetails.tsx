...
const SalesCreditNoteDetails = ({ isEditing }: { isEditing: boolean }) => {
  ...
  const { data: invoiceData, isLoading: isInvoiceLoading, isFetching: isInvoiceFetching } = Queries.useGetInvoiceDropdown({ customerId: values?.customerId, ...(isEditing && { includeId: values?.salesId }), statusFilter: "invoiced", isCreditNoteCreated: true }, !!values?.customerId);
  ...
  return (
    <Grid container spacing={2} sx={{ p: 2 }}>
      ...
      <Grid size={{ xs: 12, md: 9 }} container spacing={2}>
        <CommonValidationDatePicker name="creditNoteDate" label="Credit Note Date" required grid={{ xs: 12, md: 4 }} />

        <CommonValidationDatePicker name="dueDate" label="Due Date" grid={{ xs: 12, md: 4 }} />

        <CommonValidationSelect name="salesId" label="Select Sales" options={invoiceOptions} disabled={!values.customerId || isEditing} isLoading={isInvoiceLoading || isInvoiceFetching} grid={{ xs: 12, md: 4 }} />

        <CommonValidationSelect name="salesManId" label="Sales Person" options={salesPersonOptions} disabled={!values.customerId} isLoading={isSalesPersonLoading || isSalesPersonFetching} grid={{ xs: 12, md: 4 }} />

        <CommonValidationSelect name="productType" label="Product Type" options={SALES_CREDIT_NOTE_PRODUCT_TYPE_OPTIONS} grid={{ xs: 12, md: 4 }} />

        <CommonValidationSelect name="reverseCharge" label="Reverse Charge" options={REVERSE_CHARGE} grid={{ xs: 12, md: 4 }} />

        <CommonValidationTextField name="reason" label="Reason" placeholder="Enter reason for credit note" grid={{ xs: 12, md: 4 }} />

        <CommonValidationTextField name="sez" label="SEZ" placeholder="Enter SEZ" grid={{ xs: 12, md: 4 }} />

        <CommonValidationCheckbox name="paymentReminder" label="Payment Reminder" grid={{ xs: 12, md: 4 }} />
      </Grid>
...
export default SalesCreditNoteDetails;
