...
const SalesCreditNoteDetails = ({ isEditing }: { isEditing: boolean }) => {
  ...
  const { data: invoiceData, isLoading: isInvoiceLoading, isFetching: isInvoiceFetching } = Queries.useGetInvoiceDropdown({ customerFilter: values?.customerId, includeId: values?.salesId, statusFilter: "invoiced" }, !!values?.customerId);
