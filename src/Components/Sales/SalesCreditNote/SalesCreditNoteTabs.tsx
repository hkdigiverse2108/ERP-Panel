...
const SalesCreditNoteTabs = ({ emptyRow, isEditing }: { emptyRow: SalesCreditNoteItem; isEditing: boolean }) => {
  ...
  const { data: singleInvoice, isLoading: isSingleInvoiceLoading, isFetching: isSingleInvoiceFetching } = Queries.useGetSingleInvoice(values.salesId, Boolean(values.salesId) && !isEditing);
  ...
  useEffect(() => {
    ...
    if (values.salesId && singleInvoice?.data) {
      const existingItems = (values?.productDetails || []).filter((item) => item?.productId);
      const finalItems = [...existingItems, ...mapItems(singleInvoice?.data?.items)];
      setFieldValue("productDetails", finalItems);
    }
  }, [singleInvoice]);
  const calculateRowValues = (index: number) => {
    ...
    const taxIncluded = typeof product?.isSalesTaxIncluding === "boolean" ? product.isSalesTaxIncluding : false;
              ...
              {({ push, remove }) => {
                const columns: CommonTableColumn<any>[] = [
                  ...
                  { key: "sr", header: "#", bodyClass: "w-[50px] text-center align-middle", render: (_, i) => i + 1 },
                  {
                    ...
                    render: (_, index) => <CommonValidationSelect name={`productDetails.${index}.productId`} syncName={`productDetails.${index}.variantId`} label="Select Product" options={GenerateOptions(productsData?.data)} isLoading={isProductLoading} required disabled={!isCustomerSelected} />,
                  },
                  {
                    key: "qty",
                    header: "Qty",
                  ...
                  },
                  {
                    ...
                    header: "Price",
                    bodyClass: "min-w-28",
                    render: (_, index) => <CommonValidationTextField name={`productDetails.${index}.price`} type="number" />,
                  },
                  {
                    key: "discount1",
                    header: "Disc",
                ...
                return <CommonTable isLoading={isSingleInvoiceLoading || isSingleInvoiceFetching} showFooter data={values.productDetails || []} columns={columns} rowKey={(_row, index) => index.toString()} getRowClass={() => "align-top"} />;
              }}
            </FieldArray>
          </Box>
        </Box>
