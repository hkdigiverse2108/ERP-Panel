...
const CommonTermsAndCondition = ({ selectedTermIds, onChange, isView }: CommonTermsAndConditionProps) => {
  const [allTerms, setAllTerms] = useState<TermsConditionBase[]>([]);

  const { data: termsConditionData } = Queries.useGetTermsCondition({ all: true });
  ...
  const handleOpenAddTerm = () => {
    dispatch(setTermsAndConditionFormModal({ open: true, data: null }));
  };

  const handleEditSingleTerm = (term: TermsConditionBase) => {
    dispatch(setTermsAndConditionFormModal({ open: true, data: term }));
  };

  const handleOpenSelectModal = () => {
    dispatch(setTermsAndConditionSelectionModal({ open: true, alreadySelectedIds: selectedTermIds }));
  };
