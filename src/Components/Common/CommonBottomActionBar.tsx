...
const CommonBottomActionBar: FC<CommonBottomActionBarProps> = ({ children, isLoading, save, clear, onClear, onSave, onSaveAndNew }) => {
      ...
      <Grid container spacing={2} className="w-full! flex items-center justify-between mx-5 my-4">
        ...
        {clear && (
          <>
            ...
            <Grid sx={{ display: "flex", justifyContent: "flex-start", gap: 2 }}>
              <CommonButton type="submit" variant="contained" title="Save" onClick={onSave} loading={isLoading} disabled={isLoading} />
              <CommonButton type="submit" variant="contained" title="Save & Create New" onClick={onSaveAndNew} loading={isLoading} disabled={isLoading} />
            </Grid>
          </>
        )}
        {save && (
