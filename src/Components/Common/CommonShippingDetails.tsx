...
const CommonShippingDetails = ({ namePrefix = "shippingDetails", gridProps = {} }: CommonShippingDetailsProps) => {
    ...
    const { data: customerData, isLoading: isCustomerLoading } = Queries.useGetContactDropdown({ typeFilter: "transporter"});
    const customers = customerData?.data || [];

    return (
        <Box sx={{ display: "grid", gridTemplateColumns: { xs: "1fr", md: "repeat(4, 1fr)" }, gap: 2, ...gridProps }}>
            <CommonValidationSelect name={getFieldName("transporterId")} label="Transporter Name" options={GenerateOptions(customers)} isLoading={isCustomerLoading} />
