...
const Purchase3Jasper = forwardRef<HTMLDivElement, { bill?: SupplierBillBase | any }>(({ bill }, ref) => {
    ...
    <div ref={ref} className="w-[210mm] mx-auto bg-white text-black p-3 font-serif text-[10.5px] border">
      ...
      <div className="grid grid-cols-2 border-b">
        ...
        <div className="border-r p-2">
          ...
          <div>Company GSTIN : {supplierGst}</div>
        </div>

        {/* Info */}
        <div className="p-2">
          {[
            ["Supplier Bill No", billNo],
            ["Bill Date", billDate],
      ...
      <table className="w-full border text-[9px]">
        <thead>
          <tr>
            ...
            <th className="border p-1">Selling Price</th>
            <th className="border p-1">Discount 1</th>
            <th className="border p-1">Discount 2</th>
            ...
            items.map((item: any, i: number) => {
              ...
              return (
                <tr key={i} style={{ height: "220px" }}>
                  ...
                  <td className="border p-1 text-center align-top">{Number(item.qty || 1).toFixed(0)}</td>
                  <td className="border p-1 text-center align-top">{item.productId?.unit?.name || item.uomId?.name || "PCS"}</td>
                  <td className="border p-1 text-right align-top">{Number(item.unitCost || item.sellingPrice || item.mrp || 0).toFixed(2)}</td>
                  <td className="border p-1 text-right align-top">{Number(item.mrp || 0).toFixed(0)}</td>
      ...
      <div className="grid grid-cols-2 border mt-2">
        ...
        <div className="p-2 text-right">
          ...
          <div className="font-bold">Net Amount : {Number(summaryNetAmount).toFixed(2)}</div>
          <div className="font-bold">Payable Amount : {Number(summaryNetAmount).toFixed(2)}</div>
        </div>
      </div>
...
export default Purchase3Jasper;
